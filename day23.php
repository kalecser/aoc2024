<?php
    ...
    function countGroupsOfThreeWithHistorian($input) {
        ...
        $connections_with_historian = [];
        foreach ($connections as $computer => $others) {
            if ($_is_historian = str_starts_with($computer, 't')) {
                foreach (generatePermutationsOfTwo($others) as $perm) {
                    ...
                    $perm[] = $computer;
                    sort($perm);
                    $connections_with_historian[implode('-', $perm)] = true;
                }
            }
        }
        
        return count($connections_with_historian);
    }
    
    function generatePermutationsOfTwo($array) {
        $result = [];
        $length = count($array);
        ...
        for ($i = 0; $i < $length; $i++) {
            for ($j = 0; $j < $length; $j++) {
                if ($i !== $j) { // Ensure elements are not the same
