...
echo "Sample phase 1 result expected: 6 actual: " . countPossibleCombinations(sample()) . "\n"; 
echo "Actual phase 1 result expected: 251 actual: " . countPossibleCombinations(input()). "\n"; 
echo "Sample phase 2 result expected: 16 actual: " . countPossibleCombinations(sample(), all_combinations: true) . "\n"; 
echo "Actual phase 2 result expected: 616957151871345 actual: " . countPossibleCombinations(input(), all_combinations: true) . "\n"; 


function countPossibleCombinations($input, $all_combinations=false) {
	$lines = explode("\n", $input);
	$available_patterns_spec = $lines[0];
	$available_patterns = [];
	foreach(explode(',', $available_patterns_spec) as $pattern) {
		$available_patterns[] = trim($pattern);
	}
	
	$result = 0;
	$display_pattern_index = 2;
	for ($i = $display_pattern_index; $i < count($lines); $i++){
		
		$pattern = $lines[$i];
		$memory = [];
		
		$all_combinations_count = countPatternPossible($pattern, $available_patterns, $memory);
		$result += $all_combinations ?$all_combinations_count :$all_combinations_count>0;
	}
...
function countPatternPossible($pattern, &$available_patterns, &$memory):int {
	if (array_key_exists($pattern, $memory)) return $memory[$pattern];
	if (strlen($pattern) == 0) return 1;
	
	$count_possible = 0;
	foreach ($available_patterns as $p) {
		$starts_with = str_starts_with($pattern, $p);
		
		if ($starts_with){
			$inner_count = countPatternPossible(substr($pattern, strlen($p)), $available_patterns, $memory);
			$memory[$pattern] = $memory[$pattern] = ($memory[$pattern]??0) + (1 * $inner_count);
			$count_possible = $count_possible + (1 * $inner_count);
		}
	}
